...
6. Update the standardized pass result at `authoring-reviews/savings-goals/passes/0002-kind-aware-discovery/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Update `authoring-reviews/savings-goals/passes/0002-kind-aware-discovery/result.yaml` using its existing standardized fields.
