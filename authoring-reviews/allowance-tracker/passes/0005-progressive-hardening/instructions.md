...
6. Update the standardized pass result at `authoring-reviews/allowance-tracker/passes/0005-progressive-hardening/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Update `authoring-reviews/allowance-tracker/passes/0005-progressive-hardening/result.yaml` using its existing standardized fields.
