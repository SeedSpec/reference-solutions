...
3. Keep consequential agent inference and speculative wording outside the package under `authoring-reviews/allowance-tracker/candidates` until the author confirms it.
...
6. Update the standardized pass result at `authoring-reviews/allowance-tracker/passes/0002-kind-aware-discovery/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Apply the `application` authoring lens. Determine whether the package establishes:
...
Update `authoring-reviews/allowance-tracker/passes/0002-kind-aware-discovery/result.yaml` using its existing standardized fields.
