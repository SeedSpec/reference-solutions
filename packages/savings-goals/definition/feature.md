...
The goal-allocation record is the auditable source for progress in both modes.
In `reserved` mode it is atomically associated with the host availability
change. In `tracked` mode it is an attributed feature record and must not be
misrepresented as a host balance change.

When `allow_overfunding` is false, an allocation that would take the goal above
its target is rejected without changing history or availability. When it is
true, the underlying allocated value preserves the excess even if presentation
caps progress at complete.
...
- `keep-reserved` leaves the accepted reservation and allocation history attached to the closed goal.
- `return-available` records a reversing goal-allocation operation and releases the corresponding reservation without changing the owner's total balance.
- `record-spending` consumes the reserved value through an attributed host spending transaction and preserves its relationship to the goal.
...
Withdrawing enough value to move a reached but unclosed goal below its target
returns it to active. A closed goal cannot be withdrawn from through the core
feature.
...
The reverse `reached -> active` transition occurs when an accepted withdrawal or target increase leaves progress below the target. Archived and closed goals keep their allocation history. A closed goal is immutable except for administrative annotation.

For `max_active_goals`, both active goals and reached-but-unclosed goals count
toward the limit. Closed and archived goals do not.
...
8. Every accepted operation that changes reserved availability or the host balance records its goal state, goal-allocation record, and host transaction atomically.
9. Retrying or concurrently requesting withdrawal, completion, or archival cannot release, reserve, or spend the same units twice.

## Failure behavior

- If the host transaction cannot be recorded, allocation state and availability do not change.
- If a completion or archival disposition cannot be recorded atomically, the goal retains its prior status and fund availability.
