...
6. Update the standardized pass result at `solutions/savings-goals/authoring/passes/0002-kind-aware-discovery/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Update `solutions/savings-goals/authoring/passes/0002-kind-aware-discovery/result.yaml` using its existing standardized fields.
