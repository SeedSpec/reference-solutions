...
6. Update the standardized pass result at `solutions/savings-goals/authoring/passes/0006-agent-ready-handoff/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Update `solutions/savings-goals/authoring/passes/0006-agent-ready-handoff/result.yaml` using its existing standardized fields.
