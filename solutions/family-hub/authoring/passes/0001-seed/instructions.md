...
```sh
seedspec author record /Users/davidturner/Code/SeedSpec/reference-solutions/solutions/family-hub/seedspec --json -
...
seedspec author answer /Users/davidturner/Code/SeedSpec/reference-solutions/solutions/family-hub/seedspec --json -
  {"question_id": "...", "answer": "...", "resolution": "resolved"}
  resolution: resolved | closed | rejected | not-package-decision | routed-to-platform

seedspec author attach-source /Users/davidturner/Code/SeedSpec/reference-solutions/solutions/family-hub/seedspec --json -
  {"source": {"kind": "document", "authority": "author", "location": "...", "summary": "..."}}

seedspec author reviewed /Users/davidturner/Code/SeedSpec/reference-solutions/solutions/family-hub/seedspec --json -
...
- Run `seedspec author schema result` to inspect the durable shape these commands write. Pass `--pass 0001-seed` only when acting on a thread other than the open one.

## More depth when you need it

Each topic is served on request; none of it is required reading up front.

- `seedspec author guidance --topic review-model` — why there are four threads and what each is for
- `seedspec author guidance --topic source-boundary` — what may become a finding, and why absence is not a gap
- `seedspec author guidance --topic response` — the shape of an author-facing response
- `seedspec author guidance --topic change-loop` — proposing an edit and obtaining author acceptance
- `seedspec author guidance --topic record` — what belongs in durable pass state
- `seedspec author guidance --topic depth` — what each coaching depth changes
