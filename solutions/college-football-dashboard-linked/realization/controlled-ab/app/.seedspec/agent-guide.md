...
- Root package: org.seedspec.examples.college-football-dashboard-linked@0.2.0 (kind hint: application)
- Additions: none
- Implementation profiles: recorded
- Applied intent: affirmed
- Configuration: explicitly selected
- Optional components: org.seedspec.examples.college-football-dashboard-linked/acceptance
- Optional artifacts: none
- Task sequences: none
- Implementation resources: org.seedspec.examples.college-football-dashboard-linked/org.seedspec.guidance.context7-current-docs (instructions; available)

## Applied intent

- org.seedspec.examples.college-football-dashboard-linked: **as-authored**; package-author source `org.seedspec.intent.native` at `definition/college-football-dashboard.md`

Project-local contributions:

- **constraint** `deterministic-fixtures` [affirmed; end-user]: Use clearly labeled deterministic fixture data and never imply that it is live.
- **constraint** `no-external-data-services` [affirmed; end-user]: Do not use credentials, paid services, or external data APIs.
- **constraint** `no-deployment` [affirmed; end-user]: Build and verify the application locally, but do not deploy it.
...
### org.seedspec.examples.college-football-dashboard-linked (preferred)

- **External AI Canvas Andromeda** (`andromeda-external`) **PREFERRED**: Use the externally published AI Canvas Andromeda design system and components as the preferred visual realization without carrying their source or design guidance in this package.
  - Guidance: `implementation-profiles/org.seedspec.examples.college-football-dashboard-linked/andromeda-external/andromeda-external-profile.md`
  - Prerequisite [environment-inspection; evidence optional]: The target uses React or can faithfully adapt externally retrieved React component source without replacing a user-selected application architecture.
    - Inspect the target project before retrieving or installing external component code.
  - Tradeoff: Names the author's preferred component and design system without increasing the SeedSpec package size.
  - Tradeoff: Depends on what the external site exposes, the implementing agent's available tools, and any applicable access or licensing boundary at implementation time.
  - Tradeoff: Provides no bundled Andromeda source, tokens, rules, component mapping, or package-scoped implementation skill.
...
- org.seedspec.examples.college-football-dashboard-linked: additional guidance `none`

Resolve these resources before considering their contents:

- **AVAILABLE** org.seedspec.examples.college-football-dashboard-linked/org.seedspec.guidance.context7-current-docs@0.1.0 (instructions; exact)
...
- **complete-acceptance**: all acceptance material from org.seedspec.examples.college-football-dashboard-linked/acceptance
  - Verification plan: prove **realization** by tool-check at completion; evidence required.
  - Evidence guidance: Verify the production build, automated interaction/state checks, accessibility-oriented markup checks, and responsive layout rules against CFD-01 through CFD-26.
...
- Component org.seedspec.examples.college-football-dashboard-linked/acceptance: `components/org.seedspec.examples.college-football-dashboard-linked/acceptance/`
