...
6. Update the standardized pass result at `solutions/hubspot-daily-metric/authoring/passes/0001-concern-separation/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Update `solutions/hubspot-daily-metric/authoring/passes/0001-concern-separation/result.yaml` using its existing standardized fields.
