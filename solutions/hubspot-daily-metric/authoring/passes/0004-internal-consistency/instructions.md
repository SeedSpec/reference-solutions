...
3. Keep consequential agent inference and speculative wording outside the package under `solutions/hubspot-daily-metric/authoring/candidates` until the author confirms it.
...
6. Update the standardized pass result at `solutions/hubspot-daily-metric/authoring/passes/0004-internal-consistency/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
Update `solutions/hubspot-daily-metric/authoring/passes/0004-internal-consistency/result.yaml` using its existing standardized fields.
...
- Bundled guidance: `npx --yes @seedspec/cli@0.1.0-alpha.3 docs authoring internal-consistency`
