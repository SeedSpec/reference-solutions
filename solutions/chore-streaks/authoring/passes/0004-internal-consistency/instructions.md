...
6. Update the standardized pass result at `solutions/chore-streaks/authoring/passes/0004-internal-consistency/result.yaml`. Set `outcome: needs-author` while blocked on author judgment and `outcome: completed` only after validation.
...
- KIND_RECOMMENDED_CONCEPT_MISSING (definition): The feature definition does not clearly address configurable behavior.

## Required result

Update `solutions/chore-streaks/authoring/passes/0004-internal-consistency/result.yaml` using its existing standardized fields.
